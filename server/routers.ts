...
export const appRouter = router({
  ...
  portfolio: router({
    ...
    updateProfileCopy: protectedProcedure.input(z.object({ slug: z.string().regex(/^[a-z0-9-]+$/), displayName: z.string().max(360).optional(), bio: z.string().max(8_000).optional(), headline: z.string().max(180).optional(), tagline: z.string().max(240).optional(), timezone: z.string().max(80).optional(), availabilityStatus: z.string().max(120).optional(), currentFocus: z.string().max(240).optional(), nowStatus: z.string().max(240).optional(), learningStatus: z.string().max(240).optional(), location: z.string().max(360).optional(), websiteUrl: z.string().max(2_048).optional() })).mutation(async ({ ctx, input }) => {
      const values = {
