...
export const appRouter = router({
  ...
  billing: router({
    ...
    requestManagedDomain: protectedProcedure.input(z.object({ domain: z.string().min(4).max(255) })).mutation(async ({ ctx, input }) => { const db = await getDb(); const current = db ? (await db.select().from(subscriptions).where(eq(subscriptions.userId, ctx.user.id)).limit(1))[0] : localGet<any>(`billing:${ctx.user.id}`, { plan: "free", status: "inactive" }); if (!current || current.plan === "free" || current.status !== "active") throw new TRPCError({ code: "FORBIDDEN", message: "An active Pro or Pro+ plan is required for managed domains" }); const values = { managedDomainAddOn: true, managedDomainName: input.domain, managedDomainStatus: "requested" as const }; if (db) await db.update(subscriptions).set(values).where(eq(subscriptions.userId, ctx.user.id)); else localSet(`billing:${ctx.user.id}`, { ...current, ...values }); return values; }),
  }),
  admin: router({
