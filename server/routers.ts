...
export const appRouter = router({
  ...
  portfolio: router({
    ...
    updateProfileCopy: protectedProcedure.input(z.object({ slug: z.string().regex(/^[a-z0-9-]+$/), displayName: z.string().max(360).optional(), bio: z.string().max(8_000).optional(), location: z.string().max(360).optional(), websiteUrl: z.string().max(2_048).optional() })).mutation(async ({ ctx, input }) => {
      const values = {
      ...
      const db = await getDb();
      if (!db) { localSet(`profile:${input.slug}`, { ...localGet(`profile:${input.slug}`, demoProfile), ...values, slug: input.slug }); return values; }
      ...
      else await db.insert(profiles).values({ userId: ctx.user.id, slug: input.slug, githubLogin: input.slug, displayName: values.displayName || ctx.user.name || input.slug, bio: values.bio || null, location: values.location || null, websiteUrl: values.websiteUrl || null });
      return values;
    }),
