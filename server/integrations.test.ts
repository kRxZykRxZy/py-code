...
import { classifyProjectCategoryWithAI, generateProjectComparison, generatePortfolioNarrative, rewritePortfolioBio, suggestProjectTags, suggestProjectTitle, getGitHubContributionCalendar, getGitHubContributorCount, getGitHubUserEvents, integrationConfig, summarizeCommitActivity, summarizeRepository } from "./integrations";
...
describe("integration fallbacks", () => {
  ...
  it("suggests bounded deduplicated project tags and falls back to topics", async () => {
    vi.mocked(axios.get).mockResolvedValueOnce({ data: "TypeScript, developer-tools, TypeScript, unsafe tag!" });
    await expect(suggestProjectTags({ name: "folio", topics: ["portfolio"] })).resolves.toEqual(["typescript", "developer-tools", "unsafetag"]);
    vi.mocked(axios.get).mockRejectedValueOnce(new Error("offline"));
    await expect(suggestProjectTags({ name: "folio", topics: ["Portfolio", "tools"] })).resolves.toEqual(["portfolio", "tools"]);
  });

  it("suggests a bounded project title and falls back to the repository name", async () => {
    vi.mocked(axios.get).mockResolvedValueOnce({ data: "Calm Portfolio Workspace!" });
    await expect(suggestProjectTitle({ name: "calm-portfolio", description: "A portfolio workspace", language: "TypeScript" })).resolves.toBe("Calm Portfolio Workspace");
    vi.mocked(axios.get).mockRejectedValueOnce(new Error("offline"));
    await expect(suggestProjectTitle({ name: "calm-portfolio" })).resolves.toBe("Calm Portfolio");
  });
  ...
  it("generates a bounded AI project comparison and falls back safely", async () => {
    vi.mocked(axios.get).mockResolvedValueOnce({ data: "orbit-ui pairs a polished interface with folio-cli's focused tooling. Both projects emphasize thoughtful TypeScript experiences." });
    await expect(generateProjectComparison([{ name: "orbit-ui", description: "A UI kit", language: "TypeScript" }, { name: "folio-cli", description: "A terminal tool", language: "TypeScript" }])).resolves.toContain("orbit-ui");
    vi.mocked(axios.get).mockRejectedValueOnce(new Error("offline"));
    await expect(generateProjectComparison([{ name: "orbit-ui", language: "TypeScript" }, { name: "folio-cli", language: "TypeScript" }])).resolves.toContain("orbit-ui and folio-cli");
  });
